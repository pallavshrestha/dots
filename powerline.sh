...
__powerline() {
    ...
    ps1() {
        ...
        # Checks to see if there is a conda environment running by 
        # seeing if the CONDA_DEFAULT_ENV environmental variable is set.
        if [ ! -z $CONDA_DEFAULT_ENV ]; then
          IFS='/' read -a array <<< $CONDA_DEFAULT_ENV
          local cenv="$COLOR_SUCCESS(${array[-1]})$COLOR_RESET "
        else
          local cenv=""
        fi


        PS1="$cenv$venv$cwd$git$symbol"
